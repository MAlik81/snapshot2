...
class BusinessController extends BaseController
{
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    return $html;
                })->addColumn('biz_creator', function ($row) {

                    if (trim($row->biz_creator) ) {
                        return $row->biz_creator;
                    } else {
                        return 'Self';
                    }
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            $eventphotoModel = EventAlbumPhotos::where('business_id', $id)
            ...
            if ($eventphotoModel && count($eventphotoModel) > 0){
                foreach ($eventphotoModel as $key => $value_photo) {
                    // Delete the object from S3
                    Storage::disk('s3')->delete($value_photo['path']);
                    Storage::disk('s3')->delete($value_photo['thumbnail_path']);
                    Storage::disk('s3')->delete($value_photo['low_quality_path']);
                    // Delete the record from the database
                    EventAlbumPhotos::findOrFail($value_photo['id'])->delete();
                }
            }
            // delete event albums where business_id = $business_id
            Eventalbums::where('business_id', $id)->delete();
            // delete event where business_id = $id
            Event::where('business_id', $id)->delete();
            // delete event photo discount where business_id = $id
            EventPhotoDiscount::where('business_id', $id)->delete();
            // delete shopping cart where business_id = $id
            ShoppingCart::where('business_id', $id)->delete();
            // delete overlay image where business_id = $id
            OverlayImage::where('business_id', $id)->delete();
            // delete business where business_id = $business_id
            $all_subs = Subscription::where('business_id', $id)->get();
            foreach ($all_subs as $sub) {
                if ($sub->status == 'approved' || $sub->status == 'renewed') {
                    $sub->status = 'canceled';
                    $sub->save();
                    $sub_id = $sub->payment_transaction_id;
