...
class SuperadminSubscriptionsController extends BaseController
{
    ...
    public function index()
    {
        ...
        //     $sub_id = $value['id'];
        //     $status = $value['status'];
        //     $current_period_start = date('Y-m-d H:i:s',$value['current_period_start']); 
        //     $current_period_end = date('Y-m-d H:i:s',$value['current_period_end']);
        //     $subscription_data = Subscription::where('payment_transaction_id',$sub_id)->first();
        //     if ($subscription_data) {
        //         $subscription_data->start_date = $value['current_period_start'];
        //         $subscription_data->package_price = $value['plan']['amount']/100;
        //         $subscription_data->original_price = $value['plan']['amount']/100;
            ...
            return DataTables::of($superadmin_subscription)
                        ...
                        ->editColumn(
                            ...
                            @else($status == "declined")
                                <span class="label bg-red">{{$status}}
                                </span>
                            @endif'
                        )
                        ->editColumn(
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (!empty($input['payment_transaction_id'])) {
                $subscription_data = \Stripe\Subscription::retrieve($input['payment_transaction_id']);
                if (!$subscription_data) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if (request()->ajax()) {
            try {
                ...
                if (!empty($input['payment_transaction_id']) && $input['payment_transaction_id'] !='FREE') {
                    $subscription_data = \Stripe\Subscription::retrieve($input['payment_transaction_id']);
                    if (!$subscription_data) {
                        throw new \Exception('Invalid subscription ID');
                    }
