...
class SubscriptionController extends BaseController
{
    ...
    public function confirm($package_id, Request $request)
    {
        ...
        // dd(env('STRIPE_SECRET_KEY'));
            ...
            if ($active && $active->status != 'canceled') {
                $sub_id = $active->payment_transaction_id;
                if (isset($sub_id) && $sub_id != null) {
                    $stripr_subscription = \Stripe\Subscription::retrieve($sub_id);
                    $stripr_subscription->delete();
                }
            }
           


            $transaction_subscription = null;
            $payment_transaction_id = null;
            $stripe_customer_id = null;
            if (method_exists($this, $pay_function)) {
                $transaction_subscription = $this->$pay_function($business_id, $business_name, $package, $request);
                $payment_transaction_id = $transaction_subscription[0];
                $stripe_customer_id = $transaction_subscription[1];
            }
            ...
            Business::where('id', $business_id)->update(['stripe_customer_id' => $stripe_customer_id]);
            DB::commit();
    ...
    protected function pay_stripe($business_id, $business_name, $package, $request)
    {
        ...
        $stripe_customer_id = $business_data->stripe_customer_id; 
        if (empty($stripe_customer_id) || $stripe_customer_id == null) {
        ...
        $subscription = \Stripe\Subscription::create([
            ...
            'currency' => strtolower($currency->code),
            'items' => [
                ['price' => $package->price_id], // Replace with your actual price ID
            ],
        ]);
        return [$subscription->id, $stripe_customer_id];
    }
    ...
    public function cancelSub($subcription_id)
    {
        if (!auth()->user()->can('superadmin.access_package_subscriptions')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $subscription = Subscription::where('business_id', $business_id)
            ->with(['package', 'created_user', 'business'])
            ->find($subcription_id);
        $sub_id = $subscription->payment_transaction_id;
        ...
        $output = [
            'success' => 0,
            'msg' => 'Subscription cancled successfully. You can use the subscription benifits until end of current interval.',
        ];
    ...
    public function allSubscriptions()
    {
        ...
        $subscriptions = Subscription::where('subscriptions.business_id', $business_id)->where('subscriptions.status', '<>', 'renewed')
            ->leftjoin(
                'packages as P',
        ...
        return Datatables::of($subscriptions)
            ...
            ->addColumn('action', function ($row) {
                $actions =  '<button type="button" class="btn btn-primary btn-xs btn-modal" data-container=".view_modal" data-href="' . action([\Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'show'], $row->id) . '" ><i class="fa fa-eye" aria-hidden="true"></i> ' . __('messages.view') . '</button> <br/>';
                if ($row->status == 'approved') {
                    $actions .= ' <button type="button" class="btn btn-warning btn-xs" onclick="cancelSub(\'' . action([\Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'cancelSub'], $row->id) . '\')"  ><i class="fa fa-trash" aria-hidden="true"></i> Cancel Subscription</button>';
                }
                return $actions;
            })
